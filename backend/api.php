...
function handleStudents($method, $id, $input, $pdo) {
    switch ($method) {
        ...
        case 'POST':
            // Add new student
            $id = $input['id'] ?? uniqid();
            $stmt = $pdo->prepare("INSERT INTO students (id, photo, grNo, firstName, lastName, fatherName, religion, address, dateOfBirth, birthPlace, lastSchoolAttended, dateOfAdmission, class, section, monthlyFees, admissionFees, feesPaid, totalFees, familyId, relationship, parentId, status, academicYear, admissionTimestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    switch ($method) {
        case 'GET':
            if ($id) {
                ...
                // Decode JSON holidays for each setting
        ...
        case 'POST':
            // Add new setting
            $id = $input['id'] ?? uniqid();
            $stmt = $pdo->prepare("INSERT INTO settings (id, schoolName, schoolAddress, schoolPhone, schoolEmail, schoolWebsite, academicYear, currency, timezone, logo, holidays) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $id,
                $input['schoolName'] ?? '',
                $input['schoolAddress'] ?? '',
        ...
        case 'PUT':
            ...
            // Update setting
            $stmt = $pdo->prepare("UPDATE settings SET schoolName = ?, schoolAddress = ?, schoolPhone = ?, schoolEmail = ?, schoolWebsite = ?, academicYear = ?, currency = ?, timezone = ?, logo = ?, holidays = ? WHERE id = ?");
            $stmt->execute([
                $input['schoolName'] ?? '',
                $input['schoolAddress'] ?? '',
                $input['schoolPhone'] ?? '',
