...
function generateWorkingDays(year, month, holidays) {
  const days = [];
...
function main() {
  ...
  const octoberDays = generateWorkingDays(2025, 9, holidays);
  console.log(`October working days: ${octoberDays.length}`);
  
  // Generate working days for November 2025 (month 10)
  const novemberDays = generateWorkingDays(2025, 10, holidays);
