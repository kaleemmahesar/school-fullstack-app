...
const FeesSection = () => {
  ...
  // State for bulk payment functionality
  const [allPendingChallans, setAllPendingChallans] = useState([]);
  const [selectedPendingChallans, setSelectedPendingChallans] = useState([]);
  const [showBulkPaymentModal, setShowBulkPaymentModal] = useState(false);
  const [bulkPaymentData, setBulkPaymentData] = useState({
    paymentMethod: 'cash',
    paymentDate: new Date().toISOString().split('T')[0],
    discountAmount: 0, // Overall discount for the batch
    discountReason: ''
  });
  
  // State for individual discounts per challan
  const [challanDiscounts, setChallanDiscounts] = useState({});
  ...
  const filteredStudents = useMemo(() => studentStats.filter(student => {
    ...
    const matchesStatus = filterStatus === 'all' || 
      (filterStatus === 'paid' && student.completionRate === 100) || 
      (filterStatus === 'pending' && student.completionRate < 100);
  ...
  // Function to handle bulk payment modal
  const handleBulkPayment = () => {
    // Get all pending challans for the filtered students
    const pendingChallans = [];
    
    filteredStudents.forEach(student => {
      if (student.feesHistory && Array.isArray(student.feesHistory)) {
        student.feesHistory.forEach(challan => {
    ...
    setAllPendingChallans(pendingChallans);
    setSelectedPendingChallans(pendingChallans.map(challan => challan.id)); // Select all by default
    
    // Initialize discount amounts for each challan to 0
    const initialDiscounts = {};
    pendingChallans.forEach(challan => {
      initialDiscounts[challan.id] = 0;
    });
    setChallanDiscounts(initialDiscounts);
    
    setShowBulkPaymentModal(true);
  };
  
  // Function to submit bulk payment
  const submitBulkPayment = async () => {
    if (selectedPendingChallans.length === 0) {
      alert('No pending challans selected to pay.');
      return;
    }
    
    try {
      // Get the selected challans
      const selectedChallans = allPendingChallans.filter(challan => 
        selectedPendingChallans.includes(challan.id)
      );
      
      // Create challan updates with studentId and discount info included
      const challanUpdates = selectedChallans.map(challan => {
        const discountAmount = challanDiscounts[challan.id] || 0;
        return {
          studentId: challan.studentId,
          challanId: challan.id,
          paymentMethod: bulkPaymentData.paymentMethod,
          paymentDate: bulkPaymentData.paymentDate || new Date().toISOString().split('T')[0],
          discountAmount: discountAmount,
          discountReason: discountAmount > 0 ? bulkPaymentData.discountReason || 'Bulk payment discount' : ''
        };
      });
      
      // Dispatch the bulkUpdateChallanStatuses action
      await dispatch(bulkUpdateChallanStatuses({ challanUpdates })).unwrap();
      ...
      // Show success message
      alert(`Successfully processed ${selectedChallans.length} payment(s)!`);
      
      // Close the modal
      setShowBulkPaymentModal(false);
      setAllPendingChallans([]);
      setSelectedPendingChallans([]);
      setChallanDiscounts({});
    } catch (error) {
      console.error('Error processing bulk payments:', error);
      alert('Failed to process bulk payments. Please try again.');
    }
  };
    ...
    <>
      ...
      {showBulkPaymentModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-4xl max-h-[90vh] overflow-auto">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Bulk Payment for Pending Challans</h3>
            <p className="text-sm text-gray-600 mb-4">You have {allPendingChallans.length} pending challan(s). Select the ones you want to pay.</p>
                  ...
                  <tr>
                    <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-12">
                      <input
                        ...
                        checked={allPendingChallans.length > 0 && selectedPendingChallans.length === allPendingChallans.length}
                        onChange={(e) => {
                          if (e.target.checked) {
                            // Select all
                            setSelectedPendingChallans(allPendingChallans.map(challan => challan.id));
                          } else {
                            // Deselect all
                            setSelectedPendingChallans([]);
                          }
                    ...
                    <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Class</th>
                    ...
                    <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Discount</th>
                    <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Final Amount</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {allPendingChallans.map((challan, index) => (
                    <tr key={challan.id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 whitespace-nowrap">
                        <input
                          type="checkbox"
                          className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                          checked={selectedPendingChallans.includes(challan.id)}
                          onChange={(e) => {
                            if (e.target.checked) {
                              // Add to selected
                              setSelectedPendingChallans([...selectedPendingChallans, challan.id]);
                            } else {
                              // Remove from selected
                              setSelectedPendingChallans(selectedPendingChallans.filter(id => id !== challan.id));
                            }
                          }}
                        />
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">{challan.studentName}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">{challan.studentClass} {challan.studentSection}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">{challan.month}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">Rs {Math.round(challan.amount)}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                        <input
                          type="number"
                          min="0"
                          max={challan.amount}
                          value={challanDiscounts[challan.id] > 0 ? challanDiscounts[challan.id] : ''}
                          onChange={(e) => {
                            const value = e.target.value;
                            const discountValue = value === '' ? 0 : Math.min(parseFloat(value) || 0, challan.amount);
                            setChallanDiscounts({
                              ...challanDiscounts,
                              [challan.id]: discountValue
                            });
                          }}
                          className="w-20 px-2 py-1 border border-gray-300 rounded text-sm"
                          placeholder="0"
                          disabled={!selectedPendingChallans.includes(challan.id)}
                        />
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">Rs {Math.round((challan.amount || 0) - (challanDiscounts[challan.id] || 0))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <form onSubmit={(e) => {
              ...
              <div className="flex justify-end space-x-3 pt-4">
                ...
                <button
                  ...
                  disabled={selectedPendingChallans.length === 0}
                >
                  <FaDollarSign className="mr-2" /> Process {selectedPendingChallans.length} Payment(s)
                </button>
              </div>
            </form>
          </div>
